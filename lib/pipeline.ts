...
export async function runPipeline(
  ...
  try {
    ...
    const rawHumidity = hourly.relative_humidity_2m?.[0] ?? 0;
    const rawCloud = hourly.cloud_cover?.[0] ?? 0;
    ...
    const rawRisk = heatIndex * 0.5 + humidity * 0.3 + cloud * 0.2;
    const riskScore = Number(Math.min(100, Math.max(0, rawRisk)).toFixed(2));
